...
import com.epam.tat19.util.DBWorker;
...
public class Phonebook {
    ...
    public boolean addPhone(String phone, String id) {
        String query;

        query = "INSERT INTO `phone` (`owner`, `number`) VALUES ('" + Integer.parseInt(id) + "', '" + phone + "')";

        Integer affected_rows = this.db.changeDBData(query);

        // Если добавление прошло успешно...
        if (affected_rows > 0) {
            String phoneId = this.db.getLastInsertId().toString();

            // Добавляем номер в общий список.

            this.persons.get(id).getPhones().put(phoneId, phone);
    ...
    // Обновление записи о номере.
    public boolean updatePhone(String id, String number, String personId) {
        ...
        if (affected_rows > 0) {
            // Обновляем запись о человеке в общем списке.
            this.persons.get(personId).getPhones().put(id, number);
            return true;
        } else {
            return false;
        }
    ...
    // Удаление записи о номере.
    public boolean deletePhone(String id, String personId) {
        if ((id != null) && (!id.equals("null"))) {
            int filtered_id = Integer.parseInt(id);
            ...
            // Если удаление прошло успешно...
            if (affected_rows > 0) {
                // Удаляем запись о телефоне из списка.
                this.persons.get(personId).getPhones().remove(id);
                return true;
            } else {
                return false;
            }
    ...
    public String getPhone(String personId, String phoneId) {
        return persons.get(personId).getPhones().get(phoneId);
    }
