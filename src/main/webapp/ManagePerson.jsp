...
<form action="<%=request.getContextPath()%>/" method="post">
    ...
    <table align="center" border="1" width="70%">
        ...
        <tr>
            ...
            <td>
                <%
                    for (Map.Entry<String, String> idAndPhone : person.getPhones().entrySet()) {
                        out.write(idAndPhone.getValue());
                %>
                &nbsp;&nbsp;<a href="<%=request.getContextPath()%>/?action=editphone&id=<%=person.getId()%>&phoneid=<%=idAndPhone.getKey()%>">Редактировать</a>&nbsp;&nbsp;
                <a href="<%=request.getContextPath()%>/?action=deletephone&id=<%=person.getId()%>&phoneid=<%=idAndPhone.getKey()%>">Удалить</a>
                <br/>
                <%
                    }
                %>
